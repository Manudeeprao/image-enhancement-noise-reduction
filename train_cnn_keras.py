...
Trains a simple convolutional autoencoder on BSD68 dataset for Gaussian noise removal.
This script:
- Loads clean images from ./BSD68/ directory
...
def load_bsd68_patches(bsd68_path, patch_size=40, noise_sigma=25):
    """
    Load BSD68 dataset and extract patches.
    
    Args:
        bsd68_path: Path to BSD68 directory
    ...
    """
    bsd68_path = Path(bsd68_path)
    
    if not bsd68_path.exists():
        raise FileNotFoundError(f"BSD68 dataset not found at {bsd68_path}")
    
    # Get all PNG files
    image_files = sorted(list(bsd68_path.glob('*.png')))
    
    if len(image_files) == 0:
        raise ValueError(f"No PNG files found in {bsd68_path}")
...
def main():
    ...
    # -------- Step 1: Load Dataset --------
    print("\n" + "="*70)
    print("Step 1: Load BSD68 Dataset and Extract Patches")
    print("="*70)
    
    noisy_patches, clean_patches = load_bsd68_patches(
        bsd68_path, 
